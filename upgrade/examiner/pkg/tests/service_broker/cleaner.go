...
func (c *cleaner) clean() error {
	klog.Info("Start cleaning resources for ServiceBroker test")
	for _, fn := range []func() error{
		c.removeServiceBinding,
		c.removeServiceInstance,
		c.unregisterServiceBroker,
	} {
		err := fn()
		if err != nil {
			return err
...
func (c *cleaner) unregisterServiceBroker() error {
	if err := c.deleteServiceBroker(); err != nil {
...
func (c *cleaner) deleteServiceBroker() error {
	err := c.sc.ServiceBrokers(c.namespace).Delete(serviceBrokerName, &metav1.DeleteOptions{})
	if err != nil {
		return err
	}
	return nil
