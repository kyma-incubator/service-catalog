...
type Validator interface {
	Validate(context.Context, admission.Request, *sc.ServiceBinding, *webhookutil.TracedLogger) error
}
...
func NewAdmissionHandler() *AdmissionHandler {
	return &AdmissionHandler{
		...
		UpdateValidators: []Validator{&ReferenceDeletion{}},
	}
}
...
func (h *AdmissionHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case admissionTypes.Create:
		for _, v := range h.CreateValidators {
			if err := v.Validate(ctx, req, sb, traced); err != nil {
	...
	default:
		traced.Infof("ServiceBinding AdmissionHandler wehbook does not support action %q", req.Operation)
